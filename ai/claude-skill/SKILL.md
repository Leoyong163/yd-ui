---
name: yd-ui
description: 用内部组件库 @yd/ui 写界面（Vue 3 组件路径，或原生 HTML 类名路径）。当任务涉及这个项目的任何 UI —— 新建页面、加表单、做表格、写弹窗、调样式、改配色、加标签状态 —— 都应先加载本技能。库分两层：core 零框架层（原生 HTML 也能用）与 vue 组件层，先判断写的是哪一种，再按里面的组件清单与类名白名单写代码，不要自造组件或 CSS。触发词：@yd/ui、组件库、YD Design、写页面、加个弹窗、做个表单、标签配色、统一风格、原生 HTML、静态页。
---
